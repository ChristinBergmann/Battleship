...
for (let i = 1; i < 11; i++) {
  for (let j = 0; j < 10; j++) {
    ...
    field.id =
      document.getElementsByClassName("outrow")[i].id +
      document.getElementsByClassName("outcol")[j].id;
...
for (let i = 1; i < 11; i++) {
  for (let j = 0; j < 10; j++) {
    ...
    field2.id =
      document.getElementsByClassName("outrow2")[i].id +
      document.getElementsByClassName("outcol2")[j].id;
...
function renderGamePlayerInfos(data) {
  ...
  } else h3.innerHTML = data.Curr_name + " vs. " + data.Opp_name;
  ...
  if (data.Curr_score === undefined || data.Opp_score === undefined) {
    scoreH3.innerHTML = " / " + " : " + " / ";
  } else scoreH3.innerHTML = data.Curr_score + " : " + data.Opp_score;
  ...
  if (myShips.some((pos) => allPosition.includes(pos))) {
    ...
    if (draggableElement.classList.contains("horizontal")) {
      ...
      if (11 - myPosition < shipValue) {
        fullPosition = letters.slice(-shipValue).map((pos) => pos + myNumber);
      } else {
        fullPosition = letters
          .slice(myPosition, myPosition + shipValue)
      ...
      //*--checks if ships fit in the board--*//
      if (11 - myPosition < shipValue) {
        fullPosition = numbers.slice(-shipValue).map((pos) => myLetter + pos);
      } else {
        fullPosition = numbers
          .slice(myPosition, myPosition + shipValue)
